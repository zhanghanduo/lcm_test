...
#ifndef __exlcm_obset_t_hpp__
#define __exlcm_obset_t_hpp__

#include <string>
#include <vector>
#include "exlcm/obs_t.hpp"
...
class obset_t
{
    public:
        ...
        std::vector< exlcm::obs_t > obstacle_set;
...
int obset_t::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int obset_t::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int obset_t::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t obset_t::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* obset_t::getTypeName()
{
    return "obset_t";
}

int obset_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    for (int a0 = 0; a0 < this->num; a0++) {
        tlen = this->obstacle_set[a0]._encodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
    ...
    tlen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &this->reserved, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int obset_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    this->obstacle_set.resize(this->num);
    for (int a0 = 0; a0 < this->num; a0++) {
        tlen = this->obstacle_set[a0]._decodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
    ...
    tlen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &this->reserved, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int obset_t::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    ...
    for (int a0 = 0; a0 < this->num; a0++) {
        enc_size += this->obstacle_set[a0]._getEncodedSizeNoHash();
    }
...
uint64_t obset_t::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == obset_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)obset_t::getHash };

    uint64_t hash = 0x733642842dec2745LL +
         exlcm::obs_t::_computeHash(&cp);

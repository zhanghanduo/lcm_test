...
#ifndef __exlcm_pose_t_hpp__
#define __exlcm_pose_t_hpp__

#include <string>
#include <vector>
#include "exlcm/obstacle.hpp"
...
class pose_t
{
    public:
        ...
        std::vector< exlcm::obstacle > Obstacles;
...
int pose_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    this->Obstacles.resize(this->num);
    for (int a0 = 0; a0 < this->num; a0++) {
...
uint64_t pose_t::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == pose_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)pose_t::getHash };

    uint64_t hash = 0x6390deed9ed905d4LL +
         exlcm::obstacle::_computeHash(&cp);
